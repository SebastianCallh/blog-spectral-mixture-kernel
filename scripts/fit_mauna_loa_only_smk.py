...
def plot_prediction():
    ...
    save_plot(fig, "maunaloa_smk_fit")


def plot_spectral_density():
    fig, ax = plt.subplots(figsize=(9, 6))
    density = spectral_density(smk=model.cov, nyquist=maunaloa.nyquist)
    freq = torch.linspace(0, maunaloa.nyquist, 5000).reshape(-1, 1)
    plot_density(freq, density.log_prob(freq), ax=ax)
    save_plot(fig, "maunaloa_smk_spectral_density")


def plot_covariance():
    fig, ax = plt.subplots(figsize=(8, 7))
    plot_cov_mat(model.cov, xx=test_x, ax=ax)
    ax.set_title("Induced covariance matrix")
    save_plot(fig, "maunaloa_smk_cov_mat")
